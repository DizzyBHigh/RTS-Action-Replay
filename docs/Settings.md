...
| **Replay Renamed** | `Replay #%replayNumber% renamed to %replayTitle%.` | Message |
| **Replay Removed** | `Replay removed: %replayTitle%.` | Message |
| **Replay Rated** | `Rated %replayTitle% %replayRating%/5.` | Message |
| **Recent** | `%replayRecent%` | Dedicated Recent/Search panel |
| **Playlist** | `%replayPlaylist%` | Dedicated Playlist panel |

Every output has a **Chat** toggle. The Recent and Playlist outputs intentionally keep their Chat toggles because their overlay presentation is handled by their dedicated panels rather than the transient message system.

### Message templates

Message templates are resolved when the event occurs. The complete requester and replay context is captured at that point, before the event crosses into the central messaging action.

The available event context includes:
