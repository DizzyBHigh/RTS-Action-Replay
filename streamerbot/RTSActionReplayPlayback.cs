...
public class CPHInline
{
    ...
    public bool SetPlayerPosition()
    {
        ...
        CPH.SetArgument("replayPosition", position.Trim());
        CPH.TriggerEvent(EventName, true);
        return true;
    }
    ...
    public bool ConfirmPlayback()
    {
        ...
        var data = Load();
        var replay = ((JArray)data["replays"]).OfType<JObject>().FirstOrDefault(x => (string)x["id"] == replayId);
        ...
        if (CPH.TryGetArg("userId", out string userId) && !string.IsNullOrWhiteSpace(userId))
        {
            ...
            var users = (JObject)(replay["users"] ?? new JObject());
            replay["users"] = users;
    ...
    private void SendMessage(string type)
    {
        ...
        CPH.SetArgument("replayLogoUrl", CPH.GetGlobalVar<string>("rts.actionreplay.brandLogoUrl", true) ?? "");
        CPH.SetArgument("replayShowClapperBranding", CPH.GetGlobalVar<bool?>("rts.actionreplay.showClapperBranding", true) ?? true);
