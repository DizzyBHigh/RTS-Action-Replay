...
public class CPHInline
{
    ...
    public bool PlayReplay()
    {
        ...
        var profile = CPH.TryGetArg("replayAnimationProfileId", out string requestedProfile) && !string.IsNullOrWhiteSpace(requestedProfile)
            ? requestedProfile.Trim() : CPH.GetGlobalVar<string>(PlaybackProfileHandoffKey, false);
        ...
        var operation = new JObject {
            ...
            ["replayUrl"] = url,
            ["replayAutoplay"] = true,
            ["replayQueueEntryId"] = queueEntryId ?? "",
            ["replayUserId"] = userId ?? "",
            ["replayUserName"] = userName ?? "",
            ["replayDirector"] = creatorName,
            ["replayNumber"] = Array.IndexOf(list.ToArray(), replay) + 1,
            ["replayTitle"] = (string)replay["title"] ?? "",
            ["replayPlayedCount"] = ((int?)replay["plays"] ?? 0) + 1,
            ["replaySource"] = source,
            ["replaySourceId"] = (string)replay["sourceId"] ?? "",
        ...
        if (string.Equals(source, "YouTube", StringComparison.OrdinalIgnoreCase)) {
            operation["replayStartTime"] = (long?)replay["startTime"] ?? 0;
            operation["replayDuration"] = (int?)replay["duration"] ?? 0;
        }
        CPH.SetGlobalVar(PlayerOperationKey, operation.ToString(Newtonsoft.Json.Formatting.None), false);
        ...
        var resolved = CPH.ExecuteMethod(ResolverAction, "ResolvePlayer");
    ...
    public bool SetPlayerPosition() { if (!CPH.TryGetArg("rawInput", out string input) || string.IsNullOrWhiteSpace(input)) return false; var parts = input.Trim().Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries); if (parts.Length == 0) return false; var duration = 1000; if (parts.Length > 1 && int.TryParse(parts[1], out var requested) && requested >= 0) duration = requested; CPH.ExecuteMethod(ResolverAction, "GetPlayerPositions"); var positions = CPH.GetGlobalVar<string>(PlayerPositionsHandoffKey, false); var positionStore = string.IsNullOrWhiteSpace(positions) ? null : JObject.Parse(positions); var selectedPosition = positionStore?.Properties().FirstOrDefault(p => string.Equals(p.Name, parts[0], StringComparison.OrdinalIgnoreCase) || string.Equals((string)p.Value["tag"], parts[0], StringComparison.OrdinalIgnoreCase)); if (selectedPosition != null) CPH.SetGlobalVar("rts.actionreplay.handoff.currentPlayerPosition", selectedPosition.Name, false); CPH.SetArgument("replayCommand", "move"); CPH.SetArgument("replayPosition", parts[0]); CPH.SetArgument("replayAnimationDuration", duration); CPH.SetArgument("replayAnimationEasing", CPH.GetGlobalVar<string>("rts.actionreplay.animation.default.easing", true) ?? "ease-in-out"); CPH.SetArgument("replayPositions", positions ?? "{\"Full Screen\":{\"scale\":100,\"x\":0,\"y\":0,\"rotateX\":0,\"rotateY\":0,\"rotateZ\":0,\"fov\":90}}"); CPH.TriggerEvent(EventName, true); return true; }
