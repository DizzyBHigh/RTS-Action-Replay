...
public class CPHInline
{
    ...
    {
        var parts = Arg("rawInput").Trim().Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries); var amount = ParseAmount(ref parts); var period = string.Join(" ", parts).Trim();
        return Queue(BuildState("leaderboard", period, "leaderboard", amount > 0 ? amount : 5));
    }
    ...
    public bool RenderSearchRequest()
    {
        ...
        if (isLeaderboard)
        {
            var leaderboardEntries = results.Skip(start).Take(amount).OfType<JObject>().Select((x, i) => new JObject { ["rank"] = start + i + 1, ["creator"] = (string)x["creator"] ?? "Unknown creator", ["count"] = (int?)x["count"] ?? 0 }).ToList();
            CPH.SetArgument("replayLeaderboardEntries", new JArray(leaderboardEntries).ToString(Newtonsoft.Json.Formatting.None));
            CPH.SetArgument("replayLeaderboardPeriod", LeaderboardPeriodLabel((string)request["filter"] ?? ""));
        }
        else
        {
    ...
    {
        var now = DateTime.Now; var key = NormalizePeriod(period); if (!string.IsNullOrWhiteSpace(key) && key != "all" && key != "today" && key != "week" && key != "month" && key != "year" && !TryParseMonthYear(period, out _, out _)) return new List<JObject>();
        int? year = null; int? month = null; if (TryParseMonthYear(period, out var requestedYear, out var requestedMonth)) { year = requestedYear; month = requestedMonth; }
        ...
        return filtered.Select(x => x["creator"] as JObject).Where(x => x != null && !string.IsNullOrWhiteSpace((string)x["id"])).GroupBy(x => (string)x["id"], StringComparer.OrdinalIgnoreCase).Select(g => new JObject { ["id"] = g.Key, ["creator"] = (string)g.First()["name"] ?? g.Key, ["count"] = g.Count() }).OrderByDescending(x => (int)x["count"]).ThenBy(x => (string)x["creator"], StringComparer.OrdinalIgnoreCase);
    }
