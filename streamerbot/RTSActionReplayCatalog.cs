...
public class CPHInline
{
    ...
    public bool RecordPlayed()
    {
        var replayId = Arg("historyReplayId"); if (string.IsNullOrWhiteSpace(replayId)) return false;
        var data = Load(); var history = data["playHistory"] as JArray ?? new JArray();
        var existing = history.OfType<JObject>().FirstOrDefault(x => string.Equals((string)x["replayId"], replayId, StringComparison.OrdinalIgnoreCase));
        var count = existing == null ? 1 : Math.Max(1, (int?)existing["count"] ?? 1) + 1;
        if (existing != null) history.Remove(existing);
        history.Insert(0, new JObject { ["replayId"] = replayId, ["title"] = Arg("historyReplayTitle"), ["creator"] = Arg("historyReplayCreator"), ["count"] = count, ["lastPlayedBy"] = Arg("historyReplayRequester"), ["lastPlayedPlatform"] = Arg("historyReplayPlatform"), ["lastPlayed"] = DateTime.Now.ToString("o") });
        while (history.Count > MaxAmount()) history.RemoveAt(history.Count - 1);
        data["playHistory"] = history; Save(data); return true;
    }
    ...
    private bool ResolveStateSelection(JObject state, int index)
    {
        ...
        CPH.SetArgument("catalogSelectionReplayId", (string)results[position]["replayId"] ?? (string)results[position]["id"] ?? ""); return !string.IsNullOrWhiteSpace((string)results[position]["replayId"] ?? (string)results[position]["id"]);
    }
    ...
    {
        var data = Load(); var list = (JArray)data["catalog"] ?? new JArray(); var type = ((string)state["filterType"] ?? "all").ToLowerInvariant(); var filter = ((string)state["filter"] ?? "").Trim(); IEnumerable<JObject> query = list.OfType<JObject>();
        if (type == "lastplayed") query = ((JArray)data["playHistory"] ?? new JArray()).OfType<JObject>();
        else if (type == "search") query = query.Where(x => SearchMatch(x, filter)); else if (type == "date") query = query.Where(x => DateMatch(x, filter)); else if (type == "creator") query = query.Where(x => CreatorMatch(x, filter));
        if (type == "recent") query = query.OrderByDescending(x => ParseDate((string)x["captured"] ?? (string)x["added"])).Take(Math.Max(1, CPH.GetGlobalVar<int?>(MaxHistoryKey, true) ?? 20)); if (type == "leaderboard") query = BuildLeaderboard(query, filter); var sort = ((string)state["sort"] ?? "catalog").ToLowerInvariant(); if (type != "lastplayed" && sort == "plays") query = query.OrderByDescending(x => (int?)x["plays"] ?? 0); if (type != "lastplayed" && sort == "rating") query = query.Where(HasRatings).OrderByDescending(Rating).ThenByDescending(RatingCount); return new JArray(query);
    }
    ...
    private JObject Entry(JObject x, int number) { var creator = x["creator"] as JObject; var ratings = Ratings(x); var entry = new JObject { ["number"] = number, ["title"] = (string)x["title"] ?? "Untitled replay", ["creator"] = (string)creator?["name"] ?? (string)x["creator"] ?? "", ["plays"] = (int?)x["plays"] ?? 0, ["rating"] = ratings.Properties().Any() ? Math.Round(Rating(x), 1) : 0, ["ratingCount"] = ratings.Properties().Count() }; if (x["count"] != null) { entry["historyCount"] = (int?)x["count"] ?? 1; entry["lastPlayedBy"] = (string)x["lastPlayedBy"] ?? ""; entry["lastPlayedPlatform"] = (string)x["lastPlayedPlatform"] ?? ""; } return entry; }
    ...
    private JObject Load()
    {
        ...
        data["catalog"] = catalog; if (!(data["playHistory"] is JArray)) { data["playHistory"] = new JArray(); changed = true; } if (changed) Save(data); return data;
    }
