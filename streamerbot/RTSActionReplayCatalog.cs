...
public class CPHInline
{
    ...
    public bool RenderSearchRequest()
    {
        ...
        var entries = results.Skip(start).Take(amount).Select((x, i) => Entry(x, start + i + 1)).ToList(); CPH.SetArgument("replaySearchEntries", new JArray(entries).ToString(Newtonsoft.Json.Formatting.None)); CPH.SetArgument("replaySearchHeader", Header(request, page, pages, results.Count)); CPH.SetArgument("replaySearchRequester", (string)request["requesterName"] ?? ""); CPH.SetArgument("replaySearchParameters", Parameters(request)); CPH.SetArgument("replaySearchRequestId", (string)request["requestId"] ?? ""); CPH.SetArgument("replaySearchDuration", CPH.GetGlobalVar<int?>("rts.actionreplay.searchPanel.duration", true) ?? 10000); CPH.SetArgument("replayPanelPreset", CPH.GetGlobalVar<string>("rts.actionreplay.searchPanel.preset", true) ?? "Broadcast"); CPH.SetArgument("replayPanelPosition", CPH.GetGlobalVar<string>("rts.actionreplay.searchPanel.position", true) ?? "Centered"); CPH.SetArgument("replayPanelAnimation", PanelAnimation()); CPH.SetArgument("replayCommand", "search-panel"); CPH.TriggerEvent("RTS-Action Replay", true); return true;
    }
