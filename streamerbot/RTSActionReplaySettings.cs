...
public class CPHInline
{
 ...
 void AddImportExportSettings(RtsUI ui){ui.BeginSection("Import Settings","Settings");ui.AddTitle("Import is selective. Choose which settings to bring in from the selected settings file. Catalog import does not change play history.","Settings");ui.AddToggleSwitch("Player Settings","Import Player and playback settings.","Settings","rts.actionreplay.import.player",true);ui.AddToggleSwitch("Panel Settings","Import Panel settings.","Settings","rts.actionreplay.import.panel",true);ui.AddToggleSwitch("Message Settings","Import Message settings and message outputs.","Settings","rts.actionreplay.import.message",true);ui.AddToggleSwitch("Branding Profiles","Import Branding Profiles. After you choose the file, a profile picker shows each profile by name and ID.","Settings","rts.actionreplay.import.branding",true);ui.AddToggleSwitch("Animation Profiles","Import Animation Profiles. After you choose the file, a profile picker shows profiles grouped by component, with names and IDs.","Settings","rts.actionreplay.import.animation",true);ui.AddToggleSwitch("Visual / Design Presets","Import Visual / Design Presets.","Settings","rts.actionreplay.import.visual",true);ui.AddToggleSwitch("Title Presets","Import Title Presets.","Settings","rts.actionreplay.import.title",true);ui.AddToggleSwitch("Catalog","Import Catalog entries without changing play history.","Settings","rts.actionreplay.import.catalog",false);ui.AddDropdown("Catalog Scope","Import all platforms or selected platforms.","Settings","rts.actionreplay.import.catalog.mode",new[]{"All","Selected"},"All");ui.BeginRow();ui.AddToggleSwitch("Twitch","Include Twitch entries when Scope is Selected.","Settings","rts.actionreplay.import.catalog.twitch",true);ui.AddToggleSwitch("YouTube","Include YouTube entries when Scope is Selected.","Settings","rts.actionreplay.import.catalog.youtube",true);ui.AddToggleSwitch("Kick","Include Kick entries when Scope is Selected.","Settings","rts.actionreplay.import.catalog.kick",true);ui.AddToggleSwitch("Local","Include Local / OBS entries when Scope is Selected.","Settings","rts.actionreplay.import.catalog.local",true);ui.EndRow();ui.AddDropdown("Duplicate Entries","Skip or overwrite existing profiles and Catalog entries.","Settings","rts.actionreplay.import.duplicates",new[]{"Skip","Overwrite"},"Skip");ui.AddClickableButton("Import Settings","Import using the selections above.","Import Settings","blue","Settings",()=>CPH.ExecuteMethod("RTS - Action Replay - Core - Settings Transfer","Import"));ui.EndSection();ui.BeginSection("Export Settings","Settings");ui.AddTitle("Export always includes the complete RTS Action Replay configuration.","Settings");ui.AddClickableButton("Export Settings","Save a complete RTS Action Replay settings file.","Export Settings","blue","Settings",()=>CPH.ExecuteMethod("RTS - Action Replay - Core - Settings Transfer","Export"));ui.EndSection();ui.BeginSection("Factory Reset","Settings");ui.AddTitle("Restore built-in defaults. Catalog and play history are preserved.","Settings");ui.AddClickableButton("Factory Reset","Restore built-in defaults without deleting Catalog or play history.","Factory Reset","red","Settings",()=>CPH.ExecuteMethod("RTS - Action Replay - Core - Factory Reset","FactoryReset"));ui.EndSection();}
 ...
 Dictionary<string,RtsUIPreviewSize> BuildClapperPreviewSizes(){return new Dictionary<string,RtsUIPreviewSize>{{"Centered",new RtsUIPreviewSize(700.2,506.7)}};}
