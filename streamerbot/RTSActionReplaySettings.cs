...
public class CPHInline
{
    ...
    private void AddPositionSettings(RtsUI ui)
    {
        ...
        ui.AddClickableButton("Add New Animation Profile", "Create a new information-panel animation profile.", "Add New Animation Profile", "blue", "Information Panels", delegate
        {
    ...
    {
        CPH.SetArgument("replayCommand", string.IsNullOrWhiteSpace(position) ? "position-preview-hide" : "position-preview");
    ...
    private void PreviewPanelPosition(string position, string json)
    {
        CPH.SetArgument("replayCommand", string.IsNullOrWhiteSpace(position) ? "position-preview-hide" : "panel-position-preview");
        if (!string.IsNullOrWhiteSpace(position))
        {
            CPH.SetArgument("replayPanelPosition", position);
            CPH.SetArgument("replayPanelPositions", json ?? "{}");
    ...
    private Dictionary<string, RtsUIPreviewSize> BuildPanelPreviewSizes()
    {
        ...
        {
            ["Center"] = previewSize,
            ["Middle Right"] = previewSize,
            ["Middle Right - Off Screen"] = previewSize,
            ["Middle Right minimised"] = previewSize,
            ["Middle Right - Off Screen - Minimised"] = previewSize
        };
    }
    ...
    private void AddPanelAnimationSettings(RtsUI ui)
    {
        ...
        foreach (var item in ReadPanelAnimationProfiles())
        {
            ...
            var name = id == "default" ? "Default Profile" : CPH.GetGlobalVar<string>("rts.actionreplay.panel.animation." + id + ".name", true) ?? (string)item["name"] ?? "New Panel Profile";
    ...
    private static string[][] BuildPositionTagList(string json)
    {
        ...
        foreach (var item in positions)
        {
            ...
            if (!string.IsNullOrWhiteSpace(name)) rows.Add(new[] { name, tag });
        }
        return rows.ToArray();
    }
