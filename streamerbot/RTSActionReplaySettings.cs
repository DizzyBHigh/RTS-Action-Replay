...
public class CPHInline
{
    ...
    private void AddGeneralSettings(RtsUI ui)
    {
        ...
        ui.AddNumericTextbox("HTTP Port", "Streamer.bot HTTP Server port used to serve replay files.", "General", "rts.actionreplay.httpPort", 7474, 1, 65535); ui.EndSection();
    }
    ...
    private void AddBrandingSettings(RtsUI ui)
    {
        ...
        ui.EndSection(); ui.BeginSection("Accent Colours"); ui.BeginRow(); ui.AddColorPicker("Primary Colour", "Main Action Replay accent colour used throughout the player.", "Branding", "rts.actionreplay.titlePrimaryColor", "#0384CBFF"); ui.AddColorPicker("Secondary Colour", "Secondary Action Replay accent colour used throughout the player.", "Branding", "rts.actionreplay.titleSecondaryColor", "#101416FF"); ui.EndRow(); ui.EndSection(); ui.EndSection();
    }
    ...
    private void AddPanelAnimationProfile(RtsUI ui, string title, string id)
    {
        ...
        if (id != "default") { ui.AddTextbox("Panel Profile Name", "Display name for this information-panel animation profile.", "Information Panels", UiPrefix + "panel.profile." + id + ".name", title, false); ui.AddClickableButton("Remove Profile", "Delete this information-panel animation profile.", "Delete Profile", "red", "Information Panels", () => { CPH.SetArgument("panelProfileId", id); if (CPH.ExecuteMethod("RTS - Action Replay - Core - Animation", "RemovePanelProfile")) ui.RebuildUI(rebuilt => BuildSettings(rebuilt)); }); }
        ui.BeginRow(); AddAnimationRows(ui, "Start Sequence", "The positions and transitions used when the panel appears.", PanelKey, id, "startSequence", "Hidden Left", "Information Panels"); AddAnimationRows(ui, "End Sequence", "The positions and transitions used when the panel disappears.", PanelKey, id, "endSequence", "__PANEL_POSITION__", "Information Panels"); ui.EndRow(); ui.EndSection();
    }

    private void AddAnimationProfileSettings(RtsUI ui)
    {
        ui.AddTitle("Animation Profiles", "Positions"); ui.AddDropdown("Default Animation Profile", "Fallback profile used when no entry point has a specific profile configured.", "Positions", UiPrefix + "player.selectedProfile", BuildProfileOptions(PlayerKey), "Default");
        ui.BeginSection("Entry Point Profiles", "Positions"); ui.AddTitle("Choose which animation profile each replay entry point uses.", "Positions"); ui.BeginRow(); AddPlayerEntry(ui, "Create — OBS", "obs"); AddPlayerEntry(ui, "Create — Twitch", "twitch"); ui.EndRow(); ui.BeginRow(); AddPlayerEntry(ui, "Play — Recent", "recent"); AddPlayerEntry(ui, "Play — Catalog", "catalog"); ui.EndRow(); AddPlayerEntry(ui, "Play — Playlist", "playlist"); ui.EndSection();
        ui.AddClickableButton("Add Profile", "Create a new animation profile.", "Add Profile", "blue", "Positions", () => { if (CPH.ExecuteMethod("RTS - Action Replay - Core - Animation", "AddProfile")) ui.RebuildUI(rebuilt => BuildSettings(rebuilt)); });
        foreach (var item in ReadProfiles(PlayerKey)) { var id = (string)item["id"]; if (!string.IsNullOrWhiteSpace(id)) AddPlayerAnimationProfile(ui, (string)item["name"] ?? "Default", id); }
    }
    ...
    private string[] BuildProfileOptions(string key)
    {
        var list = new List<string>(); foreach (var item in ReadProfiles(key)) { var name = (string)item["name"]; if (!string.IsNullOrWhiteSpace(name)) list.Add(name); } if (list.Count == 0) list.Add("Default"); return list.ToArray();
    }

    private JArray ReadProfiles(string key)
    {
        var config = ReadConfig(key); var profiles = config["animationProfiles"] as JArray; if (profiles == null || profiles.Count == 0) return new JArray(new JObject { ["id"] = "default", ["name"] = "Default", ["startSequence"] = new JArray(), ["endSequence"] = new JArray() }); return profiles;
    }
    ...
    private string[] BuildPanelAnimationPositionOptions() => BuildPositionOptions(CPH.GetGlobalVar<string>("rts.actionreplay.ui.panelPositions", true), "Center");
    private string[] BuildPositionOptions(string json, string fallback)
    {
        var options = new List<string> { "Hidden Left", "Hidden Right", "Hidden Top", "Hidden Bottom" }; foreach (var item in ParsePositions(json)) { var name = (string)item.Value["name"]; if (!string.IsNullOrWhiteSpace(name) && !options.Contains(name)) options.Add(name); } if (!options.Contains(fallback)) options.Add(fallback); return options.ToArray();
    }
    ...
    private void SaveUiValue(string key, object value, bool persisted)
    {
        ...
        try
        {
            var parts = key.Substring(UiPrefix.Length).Split('.'); var configKey = parts[0] == "player" ? PlayerKey : PanelKey; var config = ReadConfig(configKey); var animation = config["animation"] as JObject ?? new JObject(); config["animation"] = animation; var text = value == null ? "" : value.ToString();
            if (parts[1] == "selectedProfile") animation["selectedProfile"] = ResolveProfileId(config, text);
            else if (parts[1] == "entry") { var entries = animation["entryPoints"] as JObject ?? new JObject(); entries[parts[2]] = ResolveProfileId(config, text); animation["entryPoints"] = entries; }
            else if (parts[1] == "profile" && parts.Length == 4 && parts[3] == "name") { var profile = FindProfile(config["animationProfiles"] as JArray, parts[2]); if (profile != null) profile["name"] = text; }
            SaveConfig(configKey, config);
        }
        catch (Exception ex) { CPH.LogWarn("RTS Action Replay: animation UI save failed: " + ex.Message); }
    }
    ...
    private Dictionary<string, RtsUIPreviewSize> BuildPanelPreviewSizes() { var size = new RtsUIPreviewSize((CPH.GetGlobalVar<int?>("rts.actionreplay.panel.width", true) ?? 500) * 640.0 / 1920.0, (CPH.GetGlobalVar<int?>("rts.actionreplay.panel.height", true) ?? 700) * 360.0 / 1080.0); return new Dictionary<string, RtsUIPreviewSize> { ["Center"] = size, ["Middle Right"] = size, ["Middle Right - Off Screen"] = size, ["Middle Right minimised"] = size, ["Middle Right - Off Screen - Minimised"] = size }; }
