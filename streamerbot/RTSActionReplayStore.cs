...
public class CPHInline
{
    ...
    public bool Initialize() { if (CPH.GetGlobalVar<string>(DataKey, true) != null) return true; Save(new JObject { ["version"] = 1, ["replays"] = new JArray() }); return true; }
    ...
    public bool AddReplay()
    {
        ...
        var data = Load(); var list = (JArray)data["replays"]; var file = Path.GetFileName(path);
        for (int i = 0; i < list.Count; i++) if (string.Equals((string)list[i]["file"], file, StringComparison.OrdinalIgnoreCase)) return true;
        ...
        var replay = new JObject { ["id"] = id, ["file"] = file, ["title"] = title, ["customTitle"] = false, ["added"] = now.ToString("o"), ["creator"] = new JObject { ["id"] = creatorId, ["name"] = creatorName }, ["plays"] = 0, ["users"] = new JObject() };
        list.Insert(0, replay); Trim(list); Save(data); CPH.LogInfo($"RTS Action Replay: added {title} ({id})"); CPH.SetArgument("replayTitle", title); SendMessage("save"); if (CPH.GetGlobalVar<bool?>("rts.actionreplay.autoPlay", true) ?? false) BroadcastReplay(replay); return true;
    }
    ...
    public bool NameReplay()
    {
        ...
        var data = Load(); var list = (JArray)data["replays"]; if (index < 1 || index > list.Count) { CPH.SendMessage($"Replay #{index} does not exist."); return false; }
    ...
    {
        var list = (JArray)Load()["replays"]; var message = ""; if (list.Count == 0) message = "The replay playlist is empty."; else for (int i = 0; i < list.Count; i++) message += (i > 0 ? " | " : "") + "#" + (i + 1) + " " + (string)list[i]["title"]; CPH.SetArgument("replayPlaylist", message); SendMessage("playlist"); return true;
    }

    public bool CreatorLeaderboard()
    {
        var list = (JArray)Load()["replays"]; var groups = new System.Collections.Generic.Dictionary<string, System.Collections.Generic.KeyValuePair<string, int>>(StringComparer.OrdinalIgnoreCase);
        for (int i = 0; i < list.Count; i++) { var creator = (JObject)list[i]["creator"]; var id = (string)creator["id"]; if (string.IsNullOrWhiteSpace(id)) continue; var name = (string)creator["name"] ?? id; if (groups.ContainsKey(id)) groups[id] = new System.Collections.Generic.KeyValuePair<string, int>(name, groups[id].Value + 1); else groups[id] = new System.Collections.Generic.KeyValuePair<string, int>(name, 1); }
    ...
    {
        var list = (JArray)Load()["replays"]; var users = new System.Collections.Generic.Dictionary<string, System.Collections.Generic.KeyValuePair<string, int>>(StringComparer.OrdinalIgnoreCase);
    ...
    private void BroadcastReplay(JObject replay)
    {
        ...
        CPH.SetArgument("replayStartPosition", GetAnimationProfileString("obsClip", "startPosition", "Full Screen"));
        ...
        CPH.SetArgument("replayAnimationEasing", GetAnimationProfileString("obsClip", "easing", "ease-in-out"));
