...
public class CPHInline
{
    ...
    public bool CreateTwitchClip()
    {
        ...
        if (!string.IsNullOrWhiteSpace(rawInput))
        {
            ...
            if (remaining[0] != '"')
            {
                ...
                if (!int.TryParse(durationText, out var requestedDuration))
                {
    ...
    private JObject AddTwitchClip(ClipData clip, bool playAfterAdd)
    {
        ...
        if (existing != null)
        {
            EnsureLocalCopyIfConfigured(data, existing, clip.Id);
            return existing;
        }

        var mode = GetPlaybackMode();
        var localPath = ModeNeedsLocalCopy(mode) ? DownloadClip(clip.Id) : null;
    ...
    {
        var url = ResolvePlaybackUrl(item);
        if (string.IsNullOrWhiteSpace(url)) { CPH.SendMessage("I couldn't get a playable Twitch clip."); return false; }
        ...
        ApplyPlayerSettings("twitchClip");
    ...
    {
        var mode = GetPlaybackMode();
        var clipId = (string)item["sourceId"];
        ...
        {
            item["file"] = Path.GetFileName(downloaded);
            item["filePath"] = downloaded;
            var data = Load(); var target = FindTwitchClip((JArray)data["catalog"], clipId); if (target != null) { target["file"] = item["file"]; target["filePath"] = item["filePath"]; Save(data); }
    ...
    {
        data["version"] = 2; data["catalog"] = data["catalog"] as JArray ?? new JArray(); data["recentIds"] = data["recentIds"] as JArray ?? new JArray(); data.Remove("replays");
    ...
    private JObject FindTwitchClip(JArray catalog, string clipId)
    {
        ...
        return catalog.OfType<JObject>().FirstOrDefault(x => string.Equals((string)x["sourceType"], "Twitch", StringComparison.OrdinalIgnoreCase) && string.Equals((string)x["sourceId"], clipId, StringComparison.OrdinalIgnoreCase));
    }
