...
public class CPHInline
{
    ...
    void ApplyPresentation(string designId,string titleId,string brandId,JObject animation,bool panel)
    {
        ...
        CPH.SetArgument("replayPanelPreset",design);CPH.SetArgument("replayPanelPrimaryColor",PanelPrimaryColor(d,b));CPH.SetArgument("replayPanelSecondaryColor",(string)b["secondaryColor"]??"#101416FF");CPH.SetArgument("replayPanelTitleFont",(string)b["font"]??"Inter");CPH.SetArgument("replayPanelTitleSize",(int?)b["fontSize"]??34);CPH.SetArgument("replayPanelTitleColor",(string)b["textColor"]??"#FFFFFFFF");CPH.SetArgument("replayPanelListColor",(string)b["textColor"]??"#FFFFFFFF");
    ...
    string PanelPrimaryColor(JObject d,JObject b){var source=((string)d["backgroundSource"]??"").Trim();if(string.Equals((string)d["id"],"cut",StringComparison.OrdinalIgnoreCase)&&string.Equals(source,"RTS Dark Blue",StringComparison.OrdinalIgnoreCase))return "#0384CBFF";return (string)b["primaryColor"]??"#0384CBFF";}
    string PanelBackgroundColor(JObject d,JObject b)=>string.Equals((string)d["id"],"cut",StringComparison.OrdinalIgnoreCase)?CutBackground(d,b):((string)d["backgroundColor"]??(string)b["secondaryColor"]??"#101416FF");
