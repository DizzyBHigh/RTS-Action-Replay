...
public class CPHInline
{
    ...
    void ApplyPresentation(string designId,string titleId,string brandId,JObject animation,bool panel)
    {
        ...
        Props("replayBroadcast",Broadcast(d,b));Props("replayCut",Cut(d,b));CPH.SetArgument("replayPanelBackgroundColor",(string)d["backgroundColor"]??(string)b["secondaryColor"]??"#101416FF");CPH.SetArgument("replayDesignPresetId",(string)d["id"]??"broadcast");CPH.SetArgument("replayTitlePresetId",(string)t["id"]??"default");CPH.SetArgument("replayBrandingPresetId",(string)b["id"]??"default");
