...
public class CPHInline
{
    ...
    public bool AddProfile()
    {
        ...
        profiles.Add(CreateProfile(id, name, false));
        player["animationProfiles"] = profiles;
        SaveConfig(PlayerKey, player);
        return true;
    }
    ...
    public bool RemoveProfile()
    {
        ...
        player["animationProfiles"] = profiles; player["animation"] = animation;
        SaveConfig(PlayerKey, player);
        return true;
    }
    ...
    public bool ApplyProfile()
    {
        ...
        var item = FindProfile(profiles, profile) ?? CreateProfile("default", "Default", true);
        ...
        CPH.SetArgument("profileId", profile); CPH.SetArgument("replayAnimationProfile", profileJson);
    ...
    public bool AddPanelProfile()
    {
        ...
        panel["animationProfiles"] = profiles; SaveConfig(PanelKey, panel); return true;
    }
    ...
    public bool RemovePanelProfile()
    {
        ...
        for (var i = profiles.Count - 1; i >= 0; i--) if (string.Equals((string)profiles[i]["id"], id, StringComparison.Ordinal)) profiles.RemoveAt(i);
        var animation = panel["animation"] as JObject ?? new JObject(); animation["entryPoints"] = ResetRemovedEntries(animation["entryPoints"] as JObject, id);
        panel["animationProfiles"] = profiles; panel["animation"] = animation; SaveConfig(PanelKey, panel); return true;
    }
    ...
    public bool ResolvePanelAnimation()
    {
        ...
        var panel = ReadConfig(PanelKey, CreatePanelDefaults()); var profiles = NormalizeProfiles(panel["animationProfiles"] as JArray);
        var animation = panel["animation"] as JObject ?? new JObject(); var entries = animation["entryPoints"] as JObject ?? new JObject();
        ...
        var start = item["startSequence"] as JArray ?? DefaultPanelStart(); var end = item["endSequence"] as JArray ?? DefaultPanelEnd();
    ...
    private JArray NormalizeProfiles(JArray source)
    {
        ...
        {
            var id = (string)token["id"]; if (string.IsNullOrWhiteSpace(id) || result.Count > 0 && FindProfile(result, id) != null) continue;
            ...
            if (id == "default") seen = true; result.Add(item);
        }
    ...
    {
        var result = new JObject(); foreach (var name in names) result[name] = ResolveProfileId(profiles, (string)source?[name]) ?? "default"; return result;
    }

    private JObject ResetRemovedEntries(JObject source, string id)
    {
        var result = source ?? new JObject(); foreach (var property in result.Properties()) if (string.Equals((string)property.Value, id, StringComparison.Ordinal)) property.Value = "default"; return result;
    }

    private string ResolveProfileId(JArray profiles, string value)
    {
        if (string.IsNullOrWhiteSpace(value)) return null; foreach (var item in profiles ?? new JArray()) { if (string.Equals((string)item["id"], value, StringComparison.Ordinal)) return value; if (string.Equals((string)item["name"], value, StringComparison.OrdinalIgnoreCase)) return (string)item["id"]; } return null;
    }
    ...
    private JObject CreateProfile(string id, string name, bool defaultProfile) => new JObject { ["id"] = id, ["name"] = name, ["startSequence"] = defaultProfile ? DefaultPlayerStart() : new JArray(new JObject { ["position"] = "Full Screen", ["duration"] = 0, ["delay"] = 0, ["easing"] = "ease-in-out" }), ["endSequence"] = defaultProfile ? DefaultPlayerEnd() : new JArray(new JObject { ["position"] = "Mini Hidden", ["duration"] = 1000, ["delay"] = 0, ["easing"] = "ease-in-out" }) };
    ...
    private JArray DefaultPlayerStart() => new JArray(new JObject { ["position"] = "Mini Hidden", ["duration"] = 0, ["delay"] = 0, ["easing"] = "ease-in-out" }, new JObject { ["position"] = "Mini Angled", ["duration"] = 1000, ["delay"] = 3000, ["easing"] = "ease-in-out" }, new JObject { ["position"] = "Mini", ["duration"] = 1000, ["delay"] = 0, ["easing"] = "ease-in-out" });
    private JArray DefaultPlayerEnd() => new JArray(new JObject { ["position"] = "Mini Hidden", ["duration"] = 1000, ["delay"] = 0, ["easing"] = "ease-in-out" });
