...
public class CPHInline
{
    ...
    public bool EnsureProfiles()
    {
        ...
        EnsureEntryPointDefaults();
        var selectedId = ResolveProfileId(CPH.GetGlobalVar<string>(SelectedProfileKey, true));
        CPH.SetGlobalVar(SelectedProfileKey, selectedId == null ? "Default" : GetProfileName(selectedId), true);
        return true;
    }
    ...
    public bool ResolveEntryPointProfile()
    {
        ...
        var key = "rts.actionreplay.animation.entry." + entryPoint.Trim().ToLowerInvariant();
        var configured = CPH.GetGlobalVar<string>(key, true);
    ...
    public bool ApplyProfile()
    {
        ...
        CPH.SetArgument("replayAnimationProfile", new JObject { ["id"] = profile, ["name"] = GetProfileName(profile), ["start"] = start, ["end"] = end }.ToString(Newtonsoft.Json.Formatting.None));
    ...
    private void EnsureEntryPointDefaults()
    {
        var points = new[] { "obs", "twitch", "recent", "catalog", "playlist" };
        foreach (var point in points) SetDefault("rts.actionreplay.animation.entry." + point, "Default");
    }
    ...
    private void ResetEntryPointProfiles(string removedId)
    {
        ...
        {
            var key = "rts.actionreplay.animation.entry." + point;
            if (string.Equals(ResolveProfileId(CPH.GetGlobalVar<string>(key, true)), removedId, StringComparison.Ordinal)) CPH.SetGlobalVar(key, "Default", true);
        }
    }
    ...
    private void EnsureProfileRegistry()
    {
        ...
        foreach (var item in profiles)
        {
            ...
            var name = CPH.GetGlobalVar<string>("rts.actionreplay.animation." + id + ".name", true) ?? (string)item["name"]; if (string.IsNullOrWhiteSpace(name)) name = "New Profile";
            cleaned.Add(new JObject { ["id"] = id, ["name"] = name }); EnsureProfile(id, name);
        }
        if (!hasDefault) cleaned.Insert(0, new JObject { ["id"] = "default", ["name"] = "Default" });
        WriteProfiles(cleaned); RemoveLegacyProfileData();
    }
    ...
    private string ResolveProfileId(string value)
    {
        ...
        foreach (var item in ReadProfiles()) if (string.Equals((string)item["name"], value, StringComparison.OrdinalIgnoreCase)) return (string)item["id"];
        return null;
    }
    ...
    private string GetProfileName(string id)
    {
        ...
        var value = CPH.GetGlobalVar<string>("rts.actionreplay.animation." + id + ".name", true); if (!string.IsNullOrWhiteSpace(value)) return value;
        foreach (var item in ReadProfiles()) if (string.Equals((string)item["id"], id, StringComparison.Ordinal)) return (string)item["name"] ?? "New Profile";
        return "New Profile";
    }
