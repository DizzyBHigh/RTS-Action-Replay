...
public class CPHInline
{
    ...
    public bool AddProfile()
    {
        ...
        profiles.Add(CreateProfile(Guid.NewGuid().ToString("N"), name)); player["animationProfiles"] = profiles; SaveConfig(PlayerKey, player); return true;
    }
    ...
    public bool RemoveProfile()
    {
        ...
        var animation = player["animation"] as JObject ?? new JObject(); animation["selectedProfile"] = ResolveProfileId(profiles, (string)animation["selectedProfile"]) ?? "default"; animation["entryPoints"] = ResetRemovedEntries(animation["entryPoints"] as JObject, id); player["animationProfiles"] = profiles; player["animation"] = animation; SaveConfig(PlayerKey, player); return true;
    }
    ...
    public bool ApplyProfile()
    {
        ...
        var player = ReadConfig(PlayerKey, CreatePlayerDefaults()); NormalizePositionConfig(player, false); var profiles = NormalizeProfiles(player["animationProfiles"] as JArray); NormalizeSequences(player, false);
        ...
        var item = FindProfile(profiles, profile) ?? CreateProfile("default", "Default"); var start = item["startSequence"] as JArray ?? new JArray(); var end = item["endSequence"] as JArray ?? new JArray(); if (start.Count == 0) start = DefaultPlayerStart(); if (end.Count == 0) end = DefaultPlayerEnd();
        var profileJson = new JObject { ["id"] = profile, ["name"] = (string)item["name"] ?? "Default", ["start"] = start, ["end"] = end }.ToString(Newtonsoft.Json.Formatting.None); var playerPositions = (player["positions"] as JObject ?? new JObject()).ToString(Newtonsoft.Json.Formatting.None);
        CPH.SetArgument("profileId", profile); CPH.SetArgument("replayAnimationProfile", profileJson); CPH.SetArgument("replayPlayerPositions", playerPositions); CPH.SetArgument("replayStartPosition", (string)start[0]["position"] ?? "Full Screen"); CPH.SetArgument("replayEndPosition", (string)end[end.Count - 1]["position"] ?? "Full Screen"); CPH.SetArgument("replayAnimationDuration", 0.5); CPH.SetArgument("replayAnimationEasing", (string)start[0]["easing"] ?? "ease-in-out");
        if (handoffRequested) { CPH.SetGlobalVar(AnimationProfileHandoffKey, profileJson, false); CPH.SetGlobalVar(PlayerPositionsHandoffKey, playerPositions, false); }
        return true;
    }
    ...
    {
        var positions = config["positions"] as JObject ?? new JObject(); var obsolete = new[] { "Hidden Left", "Hidden Right", "Hidden Top", "Hidden Bottom", "Center", "Top", "Bottom", "Top Left", "Top Right", "Bottom Left", "Bottom Right" }; foreach (var name in obsolete) positions.Remove(name); if (panel) positions.Remove("Full Screen");
        var builtIn = panel ? new JObject { ["name"] = "Centered", ["tag"] = "centered", ["scale"] = 100, ["scaleX"] = 100, ["scaleY"] = 100, ["x"] = 0, ["y"] = 0, ["rotateZ"] = 0 } : new JObject { ["name"] = "Full Screen", ["tag"] = "full-screen", ["scale"] = 100, ["scaleX"] = 100, ["scaleY"] = 100, ["x"] = 0, ["y"] = 0, ["z"] = 0, ["rotateX"] = 0, ["rotateY"] = 0, ["rotateZ"] = 0, ["fov"] = 90 };
        positions[panel ? "Centered" : "Full Screen"] = builtIn; config["positions"] = positions;
    }

    private void NormalizeSequences(JObject config, bool panel)
    {
        var positions = config["positions"] as JObject ?? new JObject(); var profiles = config["animationProfiles"] as JArray ?? new JArray();
        foreach (var token in profiles) { var profile = token as JObject; if (profile == null) continue; profile["startSequence"] = FilterSequence(profile["startSequence"] as JArray, positions); profile["endSequence"] = FilterSequence(profile["endSequence"] as JArray, positions); }
    }

    private JArray FilterSequence(JArray source, JObject positions)
    {
        var result = new JArray(); foreach (var token in source ?? new JArray()) { var row = token as JObject; var position = (string)row?["position"]; if (row != null && !string.IsNullOrWhiteSpace(position) && positions.ContainsKey(position)) result.Add(row); } return result;
    }

    private JArray NormalizeProfiles(JArray source)
    {
        var result = new JArray(); var defaultProfile = (JObject)null;
        foreach (var token in source ?? new JArray()) { var id = (string)token["id"]; if (string.IsNullOrWhiteSpace(id) || FindProfile(result, id) != null) continue; var item = new JObject { ["id"] = id, ["name"] = id == "default" ? "Default" : (string)token["name"] ?? "New Profile", ["startSequence"] = token["startSequence"] as JArray ?? new JArray(), ["endSequence"] = token["endSequence"] as JArray ?? new JArray() }; if (id == "default") defaultProfile = item; else result.Add(item); }
        result.Insert(0, defaultProfile ?? CreateProfile("default", "Default")); return result;
    }
    ...
    private JObject CreatePlayerDefaults() => new JObject { ["version"] = 1, ["positions"] = new JObject(), ["animationProfiles"] = new JArray(CreateProfile("default", "Default")), ["animation"] = new JObject { ["selectedProfile"] = "default", ["entryPoints"] = new JObject { ["obs"] = "default", ["twitch"] = "default", ["recent"] = "default", ["catalog"] = "default", ["playlist"] = "default" } } };
