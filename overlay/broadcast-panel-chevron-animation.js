...
RTSReplayPanelBroadcast.startPanelChevrons = (panel, command) => {
  ...
  const createChevron = (left, width, height, index) => {
    ...
    chevron.style.color = index % 2 ? secondary : primary;
    ...
    chevron.append(polygon); mover.append(chevron); track.append(mover);
    return mover;
  };
  ...
  const spawn = () => {
    if (!panel.classList.contains('panel-broadcast') || !panel.classList.contains('show') || track !== panel._rtsPanelChevronTrack) {
      RTSReplayPanelBroadcast.stopPanelChevrons(panel); return;
    }
    const width = pendingWidth, height = pendingHeight, spacing = pendingSpacing;
    const mover = createChevron(-width, width, height, index++);
    animate(mover, -width, width);
    pendingWidth = getWidth(); pendingHeight = getHeight(); pendingSpacing = getSpacing();
    const arm = chevronArm(width, height);
    const pitch = Math.max(1, width - arm + spacing);
    panel._rtsPanelChevronTimer = setTimeout(spawn, pitch / speed * 1000);
  };
  spawn();
};

RTSReplayPanelBroadcast.observePanelChevrons = panel => {
  if (!panel) return;
  const observer = new MutationObserver(() => {
    if (panel.classList.contains('panel-broadcast') && panel.classList.contains('show')) RTSReplayPanelBroadcast.startPanelChevrons(panel, RTSReplayPanelBroadcast.command);
    else RTSReplayPanelBroadcast.stopPanelChevrons(panel);
  });
  observer.observe(panel, { attributes: true, attributeFilter: ['class'] });
};

RTSReplayPanelBroadcast.observePanelChevrons(document.getElementById('recent-list'));
