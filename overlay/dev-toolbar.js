(() => {
  ...
  bar.innerHTML = `
    ...
    <section class="dev-section"><h3>Player</h3>
      ...
      <button data-action="title">Preview Title</button>
    </section>
    <section class="dev-section"><h3>Panel</h3>
      <button data-action="panel">Show Panel</button>
    ...
    <section class="dev-section"><h3>Clapperboard</h3>
      ...
      <div class="dev-grid"><button data-action="clapper-profile">Play Profile</button><button data-action="clapper-profile-hide">Play End</button></div>
    </section>
  ...
  const profileCommand = (profile, base) => profile ? { ...base, id: profile.id, name: profile.name, start: profile.startSequence || profile.start || [], end: profile.endSequence || profile.end || [] } : null;
  ...
  const setProfile = (command, selectId, profilesKey, outputKey) => { const profile = selectedProfile(profiles(profilesKey), document.getElementById(selectId)); const value = profileCommand(profile, command); if (value) command[outputKey] = JSON.stringify(value); return command; };
  ...
  const hidePlayer = () => { const player = RTSReplay?.player; if (!player) return; playerVisible = false; player.classList.remove('show', 'dev-player'); player.style.opacity = ''; player.style.visibility = ''; RTSReplay.frame?.classList.remove('dev-frame'); bar.querySelector('[data-action="player"]').textContent = 'Show Player'; };
  ...
  const panelPositions = { center: { scale: 100, x: 0, y: 0, rotateZ: 0 }, top: { scale: 85, x: 0, y: 26, rotateZ: 0 }, bottom: { scale: 85, x: 0, y: -26, rotateZ: 0 }, left: { scale: 80, x: -27, y: 0, rotateZ: 0 }, right: { scale: 80, x: 27, y: 0, rotateZ: 0 }, 'top-left': { scale: 72, x: -25, y: 22, rotateZ: 0 }, 'top-right': { scale: 72, x: 25, y: 22, rotateZ: 0 }, 'bottom-left': { scale: 72, x: -25, y: -22, rotateZ: 0 }, 'bottom-right': { scale: 72, x: 25, y: -22, rotateZ: 0 } };
  const buildPanelCommand = () => { const command = { ...currentCommand(), replayRecent: 'dev-preview', replayRecentData: JSON.stringify(Array.from({ length: 8 }, (_, i) => ({ number: String(i + 1).padStart(2, '0'), title: ['FIRST TEST — REPLAY CAPTURE', 'GTA V — ACTION REPLAY', 'CINEMATIC DRIVE', 'NIGHT SHIFT', 'HIGHWAY RUN', 'MISSION COMPLETE', 'STREAM HIGHLIGHT', 'LAST CALL'][i], avatarUrl: '' }))) }; command.replayPanelPreset = document.getElementById('rts-dev-panel-style').value; command.replayPanelPosition = 'Preview'; command.replayPanelPositions = JSON.stringify({ Preview: panelPositions[document.getElementById('rts-dev-panel-position').value] }); return setProfile(command, 'rts-dev-panel-profile', 'replayPanelAnimationProfiles', 'replayPanelAnimation'); };
  ...
  const hidePanel = () => { const panel = RTSReplay?.recentList; if (!panel) return; panelVisible = false; RTSInformationPanelAnimation.cancel(); panel.classList.remove('show'); panel.setAttribute('aria-hidden', 'true'); bar.querySelector('[data-action="panel"]').textContent = 'Show Panel'; };
  const playPanelProfile = () => { const panel = RTSReplay?.recentList; if (!panel) return; const command = buildPanelCommand(); panel._rtsPanelAnimationCommand = command; RTSInformationPanels.show(panel, command, command.replayPanelPosition); panelVisible = true; };
