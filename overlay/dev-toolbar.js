(() => {
  ...
  const showClapper = () => { const command = setProfile(currentCommand(), 'rts-dev-clapper-profile', 'replayClapperAnimationProfiles', 'replayClapperAnimation'); RTSReplay.command = command; RTSReplayVideo.currentCommand = command; RTSReplayMessages.messageText.textContent = command.replayMessage || 'CLAPPERBOARD ANIMATION TEST'; clapperVisible = true; updateClapper(command); RTSReplayMessages.showMessage(command); bar.querySelector('[data-action="clapper"]').textContent = 'Hide Clapperboard'; };
  const hideClapper = () => { const command = currentCommand(); clapperVisible = false; RTSReplayMessages.hideMessage(command); bar.querySelector('[data-action="clapper"]').textContent = 'Show Clapperboard'; };
