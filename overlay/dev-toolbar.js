(() => {
  ...
  bar.innerHTML = `
    ...
    <section class="dev-section"><h3>Message</h3>
      <div class="dev-grid"><button data-action="message">Show Message</button></div>
      <label>Message<input id="rts-dev-message" value="MESSAGE TEST PREVIEW"></label>
    </section>
  ...
  let playerVisible = false, panelVisible = false, messageVisible = false, clapperVisible = false;
  ...
  const playPlayerProfile = () => { const command = applyPlayerProfile(), profile = parse(command.replayAnimationProfile); showPlayer(); if (profile?.start?.length) RTSReplayVideo.runAnimationProfile(command); };
  const playPlayerEnd = () => { const command = applyPlayerProfile(), profile = parse(command.replayAnimationProfile); showPlayer(); if (profile?.end?.length) RTSReplayVideo.runEndAnimationProfile(command, hidePlayer); };
  ...
  const showMessage = () => {
    ...
    if (!card) return;
    const command = { ...currentCommand(), replayMessage: document.getElementById('rts-dev-message').value.trim() || 'MESSAGE TEST PREVIEW' };
    RTSReplay.command = command;
    RTSReplayMessages.showMessage(command);
    clearTimeout(RTSReplay.messageTimer);
    messageVisible = true;
    bar.querySelector('[data-action="message"]').textContent = 'Hide Message';
  };
  const hideMessage = () => {
    const card = RTSReplayMessages?.messageCard;
    if (!card) return;
    ...
    bar.querySelector('[data-action="message"]').textContent = 'Show Message';
  };
